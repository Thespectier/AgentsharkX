...
Connect、Trust、Protect 和 Audit 的子界面入口统一位于左侧边栏。进入某个工作区后，
其一级入口下会展开当前工作区的全部子入口；当前子界面会显示高亮状态。桌面端折叠
侧边栏时仅保留一级图标，重新展开后仍按当前 URL 恢复子入口和选中状态。移动端点击
顶部菜单按钮即可打开完整侧边栏，桌面端保存的折叠偏好不会隐藏移动端子入口。子入口
切换使用前端路由，无需刷新页面，复制或重新打开具体 URL 也会回到同一子界面。

顶部栏的语言按钮显示 **中文** 或 **EN**。点击后会立即切换侧边栏、页面标题、通用
操作和主要业务内容，并将选择保存在当前浏览器中；刷新页面或切换子界面不会恢复为
默认语言。该偏好不包含凭据，也不会发送给上游管理面。

所有面向用户的时间统一按北京时间 `Asia/Shanghai` 显示，并明确标注 `UTC+8`。
Home 的问候语也按当前北京时间自动选择早上、下午、晚上或深夜，不受操作系统或
浏览器所在时区影响。BFF 和上游 API 仍保留原始 ISO 8601 时间点，时区转换只发生在
展示层。
...
在 **Protect → Runtime rules** 中点击 **New rule** 后，应依次选择显式 Agent、
填写单条规则源码、执行语法检查、填写操作备注并勾选确认。弹窗会在较矮视口中保持
操作区可见并允许内容区滚动；取消或关闭后会清除校验令牌、备注和确认状态，重新打开
必须重新检查，避免误用上一次未发布草稿的授权状态。
...
- Latency 图基于 agentgateway 最近最多 500 条脱敏请求日志计算每个桶的
  ...
  全量延迟直方图。
- 横轴和悬停时间均使用北京时间，并明确标注 `UTC+8`；页面顶部的 **Last 60m**
  与实际查询范围一致。
