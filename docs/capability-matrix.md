...
| Providers and models | agentgateway | partial | Phase 3 adapter plus 2026-07-27 runtime verification of string, reference, and custom provider shapes over `/api/config`; no dedicated read API | Normalize explicit `llm` config only; expose the custom discriminator without returning its formats or params, and preserve direct/virtual kind, source, fetched time, and raw reference. |
...
| Gateway analytics | agentgateway | supported | Phase 6 adapter/window tests plus bundled SQLite runtime aggregate | Use twelve five-minute buckets over the shared exact 60-minute range; calculate per-bucket nearest-rank P95 from the bounded 500-record redacted log sample, expose sample counts, and keep no-sample latency null. Surface explicit capability failure while peer-source data remains available. |
